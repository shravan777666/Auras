...
export const getDashboard = asyncHandler(async (req, res) => {
  ...
  if (salonInfoWithUrls.profileImage) {
    salonInfoWithUrls.profileImage = getFileUrl(salonInfoWithUrls.profileImage, req);
  }
  
  // Handle salonImage
  if (salonInfoWithUrls.salonImage) {
    salonInfoWithUrls.salonImage = getFileUrl(salonInfoWithUrls.salonImage, req);
  }
  
  // Handle documents
  if (salonInfoWithUrls.documents) {
    salonInfoWithUrls.documents = convertDocumentsToUrls(salonInfoWithUrls.documents, req);
  }
  ...
  if (salonInfoWithUrls.staff && Array.isArray(salonInfoWithUrls.staff)) {
    salonInfoWithUrls.staff = salonInfoWithUrls.staff.map(staff => {
      if (staff.profilePicture) {
        return { ...staff, profilePicture: getFileUrl(staff.profilePicture, req) };
      }
      return staff;
    });
  }
...
export const getProfile = asyncHandler(async (req, res) => {
  ...
  if (salon.documents) {
    salon.documents = convertDocumentsToUrls(salon.documents, req);
  }
...
export const getSalonStaff = asyncHandler(async (req, res) => {
  ...
  const staff = staffDocs.map(s => {
    ...
    return {
      ...
      documents: {
        governmentId: documents.governmentId ? getFileUrl(documents.governmentId, req) : null,
        certificates: Array.isArray(documents.certificates) ? documents.certificates.map(p => getFileUrl(p, req)) : []
      },
